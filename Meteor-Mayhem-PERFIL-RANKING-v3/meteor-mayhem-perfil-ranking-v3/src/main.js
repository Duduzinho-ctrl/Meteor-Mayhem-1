...
(function(){
  ...
  const LB_PREFIX = 'mm-lb2:';   // prefixo de ranking local/adapter
  ...
  let player = {
    ...
    totalRuns: 0, totalCoins: 0, bestCombo: 1, dailyStreak: 0, lastDailyDate: null
  };

  async function loadPlayer(){
    try{
      const r = await storageGet(ME_KEY);
      if(r && r.value){
        ...
        player = {
          ...
          bestCombo: Math.max(1, d.bestCombo || 1),
          dailyStreak: d.dailyStreak || 0,
          lastDailyDate: d.lastDailyDate || null
        };
      }
  ...
  async function savePlayer(){
    try{ await storageSet(ME_KEY, JSON.stringify(player)); }
  ...
  function recordRunStats(){
    ...
    player.totalCoins = (player.totalCoins||0) + coinsCollected;
    player.bestCombo = Math.max(player.bestCombo||1, maxComboThisRun);
    if(mode === 'daily'){
      const today = todayKey();
      const diff = dateDiffUTC(player.lastDailyDate, today);
      if(diff === 1) player.dailyStreak = (player.dailyStreak||0) + 1;
      else if(diff !== 0) player.dailyStreak = 1;
      player.lastDailyDate = today;
      modeDailyTab.textContent = '🔥 Diário · '+player.dailyStreak+'d';
    }
  }

  function challengeUrl(score){
    try{ return new URL(`?beat=${Math.max(0, Math.floor(score))}`, location.href).href; }
    catch(e){ return ''; }
  }

  let challengeTarget = 0;
  try{ challengeTarget = Math.max(0, Number(new URLSearchParams(location.search).get('beat')) || 0); }catch(e){}
  ...
  async function submitScore(score, prefix, elapsedSeconds){
    ...
    try{
      await storageSet(key, JSON.stringify({ name: player.name, score }));
  ...
  async function loadLeaderboardTop(limit, prefix){
    ...
    let keys = [];
    try{
      const r = await storageList(prefix);
  ...
  async function getMyRank(prefix){
    ...
    const gap = me && above ? Math.max(0, above.score - me.score) : 0;
    return { rank: idx>=0 ? idx+1 : null, top: all.slice(0,50), me, above, gap, total: all.length };
  }
  ...
  function boardRow(e, rank, highlightId){
    const row = document.createElement('div');
    row.className = 'board-row' + (rank===1?' top1':rank===2?' top2':rank===3?' top3':'') + (e.id===highlightId?' me':'');
    const medal = rank===1?'🥇':rank===2?'🥈':rank===3?'🥉':('#'+rank);
    row.innerHTML = `<div class="rank">${medal}</div><div class="name">${escapeHtml(e.name)}${e.id===highlightId?' <span class="you-badge">VOCÊ</span>':''}</div><div class="pts">${e.score}</div>`;
    return row;
  }

  async function renderBoard(list, highlightId, prefix){
    const all = list || await loadLeaderboardTop(LB_FETCH_CAP, prefix);
    boardBody.innerHTML = '';
    if(!all || all.length === 0){
    ...
    top.forEach((e,i)=> boardBody.appendChild(boardRow(e,i+1,highlightId)));
    const myIndex = all.findIndex(e=>e.id===highlightId);
    if(myIndex > 49){
      const sep = document.createElement('div');
      sep.className='board-separator'; sep.textContent='• • •'; boardBody.appendChild(sep);
      boardBody.appendChild(boardRow(all[myIndex], myIndex+1, highlightId));
      const above = all[myIndex-1];
      if(above){
        const gap = Math.max(0, above.score-all[myIndex].score);
        const gapEl=document.createElement('div'); gapEl.className='board-gap';
        gapEl.textContent = gap ? `Faltam ${gap} pontos para ultrapassar ${above.name}` : 'Você está empatado com o jogador acima';
        boardBody.appendChild(gapEl);
      }
    }
  }
  function updateProfileRank(rankData){
    if(!profileRankLine) return;
    if(rankData && rankData.rank){
      profileRankLine.textContent = `🏆 #${rankData.rank} no mundo${rankData.total ? ` · ${rankData.total} pilotos` : ''}`;
      if(rankData.above && rankData.gap){
        profileNext.textContent = `🚀 Faltam ${rankData.gap} pontos para ultrapassar ${escapeHtml(rankData.above.name)}.`;
      } else if(rankData.rank === 1){
        profileNext.textContent = '👑 Você está no topo do mundo!';
      } else {
        profileNext.textContent = '🔥 Continue jogando para subir no ranking.';
      }
    } else {
      profileRankLine.textContent = 'Ainda não entrou no ranking';
      profileNext.textContent = 'Jogue uma partida para registrar seu primeiro recorde.';
    }
  }

  async function renderProfile(){
    profileName.textContent = player.name || 'Piloto';
    statBest.textContent = (player.best||0).toLocaleString('pt-BR');
    statRuns.textContent = (player.totalRuns||0).toLocaleString('pt-BR');
    statCoins.textContent = (player.totalCoins||0).toLocaleString('pt-BR');
    statCombo.textContent = 'x'+Math.max(1, player.bestCombo||1);
    statStreak.textContent = (player.dailyStreak||0)+' dia'+((player.dailyStreak||0)===1?'':'s');
    statSkins.textContent = String((player.skins||['default']).length);
    const rankData = player.id && player.best > 0 ? await getMyRank(LB_PREFIX) : {rank:null};
    updateProfileRank(rankData);
  }
  ...
  closeProfileBtn.addEventListener('click', ()=>{
    sfx.click(); profileOverlay.classList.add('hidden');
  ...
  boardToggle.addEventListener('click', async ()=>{
    ...
    if(open){
      if(mode === 'daily'){
        ...
        const prefix = DAILY_PREFIX + todayKey() + ':';
        const data = await getMyRank(prefix);
        await renderBoard(null, player.id, prefix);
        updateProfileRank(data);
      } else {
        boardTitle.textContent = '🏆 Ranking mundial';
        boardBody.innerHTML = '<div class="board-loading">carregando ranking...</div>';
        const data = await getMyRank(LB_PREFIX);
  ...
  async function claimName(name, playerId){
    ...
    try{
      await storageSet(key, JSON.stringify({ ownerId: playerId, display: name.trim() }));
  ...
  async function submitAndShowBoard(score, prefix, label, elapsedSeconds){
    ...
    const rankData = await getMyRank(prefix);
    boardTitle.textContent = label || '🏆 Ranking mundial';
    await renderBoard(rankData.top.length >= 50 ? rankData.top : null, player.id, prefix);
    // Re-render with the full list when the player is outside the Top 50.
    if(rankData.rank && rankData.rank > 50){
      const all = await loadLeaderboardTop(LB_FETCH_CAP, prefix);
      await renderBoard(all, player.id, prefix);
    }
  ...
  async function finalizeCrash(){
    ...
    if(mode === 'endless'){
      ...
      shareBtn.dataset.text = `🚀 Fiz ${score} pontos no Meteor Mayhem e fiquei em #${rank||'?'} no ranking mundial! Tenta bater meu recorde! ${challengeUrl(score)}`;
      ...
      shareBtn.dataset.text = `🔥 Fiz ${score} pontos no Desafio Diário do Meteor Mayhem e fiquei em #${rank||'?'} hoje! Streak: ${player.dailyStreak||1} dia(s). Tenta bater! ${challengeUrl(score)}`;
      ...
      shareBtn.dataset.text = `🏁 Cheguei no nível ${currentLevel+1} do Meteor Mayhem com ${score} pontos! Tenta superar: ${challengeUrl(score)}`;
  ...
  function startGame(){
    ...
    if(mode === 'levels'){
      ...
      const controls = 'toque / ESPAÇO / ↑ para pular';
      hint.textContent = challengeTarget > 0 ? ('🎯 Desafio: bata '+challengeTarget+' pontos — '+controls) : controls;
    }
  ...
  modeDailyTab.addEventListener('click', ()=>{
    ...
    modeDailyTab.textContent = '🔥 Diário'+(player.dailyStreak ? ' · '+player.dailyStreak+'d' : '');
  });
  ...
  (async function boot(){
    ...
    if(player.dailyStreak) modeDailyTab.textContent = '🔥 Diário · '+player.dailyStreak+'d';
    if(challengeTarget > 0) modeEndlessTab.textContent = '♾️ Endless · desafio';
    // Primeiro garante que o recorde salvo foi enviado; só depois desenha o ranking.
    // Isso evita a corrida em que a tela mostra "Ninguém pontuou ainda" antes do envio terminar.
    if(player.id && player.name && player.best > 0){
      await submitScore(player.best, LB_PREFIX);
    }
    await renderBoard(null, player.id, LB_PREFIX);
